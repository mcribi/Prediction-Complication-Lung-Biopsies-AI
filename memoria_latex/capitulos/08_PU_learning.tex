\chapter{Positive-Unlabeled Learning}\label{chap:pu_learning}

Este capítulo estudia la aplicación de PU learning a la predicción de hemorragia y neumotórax. Las formulaciones evaluadas se comparan con el aprendizaje supervisado sobre representaciones tabulares y volúmenes tridimensionales, analizando tanto su rendimiento como sus principales limitaciones.

\section{Objetivo y planteamiento del análisis}

% El análisis se ha planteado como una extensión de los experimentos supervisados anteriores para estudiar la posible incertidumbre asociada al registro de las complicaciones. En el enfoque convencional, los pacientes sin una complicación registrada se consideran negativos. Sin embargo, esta ausencia de registro no permite descartar por completo que algunos casos positivos no hayan sido identificados. Por este motivo, se ha evaluado también el problema desde el marco de PU learning descrito en la Sección~\ref{sec:fundamentos_pu}, considerando como positivos los casos confirmados y como no etiquetados los restantes.
...
La comparación ha incluido cuatro formulaciones: dos referencias supervisadas y dos métodos PU específicos. Las referencias supervisadas han sido el aprendizaje convencional, que considera negativos todos los casos sin la complicación analizada, y el aprendizaje supervisado con etiquetado negativo ingenuo, que utiliza como negativos únicamente los pacientes pertenecientes al estado derivado sin complicaciones, correspondiente a \((0,0)\). Los métodos PU evaluados han sido la corrección de Elkan-Noto y Bagging PU, que incorporan un tratamiento específico del conjunto no etiquetado.
...
Una forma de estudiar esta hipótesis en trabajos futuros sería ocultar de manera controlada la etiqueta de una parte de los positivos conocidos únicamente durante el entrenamiento, manteniendo intactas sus etiquetas originales en el conjunto de prueba de cada fold. El análisis podría repetirse con distintos porcentajes de ocultación y diferentes selecciones de pacientes para comprobar si PU learning se degrada menos que el aprendizaje supervisado cuando las etiquetas positivas son incompletas. También podrían evaluarse mecanismos de ocultación no uniformes, asignando distintas probabilidades de ocultación en función de si el paciente presenta otra complicación registrada. De este modo, sería posible analizar tanto la robustez de los métodos ante distintos grados de incertidumbre como su dependencia respecto al posible mecanismo de registro. En cualquier caso, se trataría de un análisis de sensibilidad y no de una demostración de que existan positivos ocultos en la cohorte actual.
