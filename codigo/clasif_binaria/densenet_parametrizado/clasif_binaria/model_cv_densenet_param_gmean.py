...
def cross_validate(
    ...
    device='cuda',
    epochs=10,
    save_path_prefix="modelo_densenet_param_200pacientes_", 
    weight_decay=1e-5,
    dropout_prob=0.4, 
    seed=42
):
...
for seed in seeds_to_try:
    ...
    for prep in preprocessing_dirs:
        ...
        dataset = LungCTMaskedNPYDataset(
            ...
            nodule_gain=0.0          #ir probando de 0.2 a 0.5
        )
        print(f"[INFO] CSV tiene {len(labels_dict_numeric)} pacientes con etiqueta.")
        print(f"[INFO] Para preprocesado '{prep}' en '{base_dir}': "
              f"{len(dataset)} pacientes con volúmenes + máscaras + etiqueta.")
        # if len(dataset) > 0:
        #     print(f"[INFO] Ejemplos de IDs usados: {dataset.patients[:5]}")
        # else:
        #     print(f"[WARN] Dataset vacío para '{prep}'. Se salta este preprocesado.")
        #     continue  # evita el ValueError de StratifiedKFold
                ...
                for weight_decay in weight_decays_to_try:
                    for dropout_prob in dropout_probs_to_try:
                        ...
                        df_result = cross_validate(
                            ...
                            device=device,
                            epochs=EPOCHS,
                            save_path_prefix=f"modelo_densenet_param_200pacientes_{prep}_bs{bs}_lr{lr}_wd{weight_decay}_drop{dropout_prob}_seed{seed}",
...
final_results.to_csv("resultados_modelo_densenet_param_200pacientes.csv", index=False)
print("\n Todos los resultados guardados en 'resultados_modelo_densenet_param_200pacientes.csv'")