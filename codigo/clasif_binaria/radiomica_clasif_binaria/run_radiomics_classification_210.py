...
def prepare_feature_columns(df: pd.DataFrame, target_cols: List[str]) -> List[str]:
    ...
    feature_cols = [col for col in df.columns if not is_metadata_column(col)]
    if not feature_cols:
...
def wrap_multilabel_classifier(base_classifier, multilabel_strategy: str):
    ...
    if multilabel_strategy == "classifier_chain":
        return ClassifierChain(base_classifier)
...
def build_pipeline(
    ...
    wrapped_classifier = wrap_multilabel_classifier(base_classifier, multilabel_strategy=multilabel_strategy)
...
def compute_label_metrics(
    ...
    for idx, label_name in enumerate(target_cols):
        ...
        if y_score is not None and y_score.ndim == 2 and idx < y_score.shape[1]:
            try:
                row["roc_auc"] = float(roc_auc_score(y_true[:, idx], y_score[:, idx]))
            except Exception:
                row["roc_auc"] = None
        else:
            row["roc_auc"] = None
...
def aggregate_fold_metrics(fold_metrics_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    optional_cols = ["roc_auc_macro", "roc_auc_micro"]
