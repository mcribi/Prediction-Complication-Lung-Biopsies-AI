...
RUNS_ROOT = PROJECT_ROOT / "codigo/DL_multietiqueta/two_outputs_derived/multimodal/runs"
...
def load_pinned_volume_helper(manifest: dict):
    ...
    module.TARGET_LABELS = list(TARGET_LABELS)
    return module
...
def prediction_frame(
    ...
    for index, label in enumerate(TARGET_LABELS):
        ...
        frame[f"pred_fixed_{label}"] = (probabilities[:, index] >= DEFAULT_THRESHOLD).astype(int)
        frame[f"pred_tuned_{label}"] = (probabilities[:, index] >= tuned_thresholds[index]).astype(int)
        frame[f"threshold_tuned_{label}"] = float(tuned_thresholds[index])
    frame["true_Sin_complicacion"] = derive_no_complication(targets.astype(int))
    frame["pred_fixed_Sin_complicacion"] = derive_no_complication(
        frame[[f"pred_fixed_{label}" for label in TARGET_LABELS]].to_numpy()
    )
    frame["pred_tuned_Sin_complicacion"] = derive_no_complication(
        frame[[f"pred_tuned_{label}" for label in TARGET_LABELS]].to_numpy()
    )
    return frame
...
def build_run_configuration(args: argparse.Namespace, manifest: dict) -> dict:
    code_files = [
        ...
        MODULE_DIR.parent / "two_output_metrics.py",
    ]
    return {
        ...
        "dropout": DROPOUT,
        "threshold_reference": DEFAULT_THRESHOLD,
        ...
        "evaluation_scope": "exploratory_retrospective",
        "selection_limitation": "The historical top20 image configurations were selected using outer-fold image results; multimodal outer metrics are not a fully independent nested estimate.",
        "seed": SEED,
...
def train_fold(
    ...
    threshold_selection = select_per_label_thresholds(val_targets, val_prob, THRESHOLD_GRID)
    ...
    test_loss, test_prob, test_targets, test_ids, _ = evaluate(
        model,
        loaders["test_outer"],
        criterion,
        mode,
    ...
    prediction_frame(fold, val_ids, val_prob, val_targets, tuned_thresholds).to_csv(
        fold_dir / "val_predictions_best_epoch.csv",
        index=False,
    )
    prediction_frame(fold, test_ids, test_prob, test_targets, tuned_thresholds).to_csv(
        fold_dir / "test_predictions_best_epoch.csv",
        index=False,
    )
    fixed_test_metrics, _, _ = compute_two_output_and_derived_metrics(
        test_targets, test_prob, [DEFAULT_THRESHOLD, DEFAULT_THRESHOLD]
    )
    tuned_test_metrics, _, _ = compute_two_output_and_derived_metrics(
        test_targets, test_prob, tuned_thresholds
    )
    (fold_dir / "test_metrics_fixed_0_5.json").write_text(
        json.dumps(fixed_test_metrics, indent=2), encoding="utf-8"
    )
    (fold_dir / "test_metrics_tuned.json").write_text(
        json.dumps(tuned_test_metrics, indent=2), encoding="utf-8"
    )
    summary = {
        "run_config_id": run_config_id,
        "mode": mode,
        ...
        "best_val_f1_micro_fixed_0_5": best_score,
        "val_loss": val_loss,
        "test_loss": test_loss,
        ...
        "patience": patience,
        **{f"val_fixed_{key}": value for key, value in val_metrics.items()},
        **{f"fixed_{key}": value for key, value in fixed_test_metrics.items()},
        **{f"tuned_{key}": value for key, value in tuned_test_metrics.items()},
    }
...
def aggregate_mode(
    ...
    fixed_metrics, _ = compute_metrics_from_predictions(targets, probabilities, fixed_predictions)
    tuned_metrics, _ = compute_metrics_from_predictions(targets, probabilities, tuned_predictions)
    oof.to_csv(run_root / mode / "oof_predictions.csv", index=False)
    (run_root / mode / "oof_metrics_fixed_0_5.json").write_text(
        json.dumps(fixed_metrics, indent=2), encoding="utf-8"
    )
    (run_root / mode / "oof_metrics_tuned.json").write_text(
        json.dumps(tuned_metrics, indent=2), encoding="utf-8"
    )
    fold_summaries = []
    for fold in range(1, 6):
    ...
    fold_frame.to_csv(run_root / mode / "fold_summary.csv", index=False)
    result = {"mode": mode, "n_oof": len(oof)}
    result.update({f"fixed_oof_{key}": value for key, value in fixed_metrics.items()})
    result.update({f"tuned_oof_{key}": value for key, value in tuned_metrics.items()})
    metric_columns = [
        column for column in fold_frame.columns
        if column.startswith(("fixed_", "tuned_"))
        and pd.api.types.is_numeric_dtype(fold_frame[column])
    ]
    for column in metric_columns:
        result[f"mean_{column}"] = float(fold_frame[column].mean())
        result[f"std_ddof1_{column}"] = float(fold_frame[column].std(ddof=1))
    result["num_folds_completed"] = int(len(fold_frame))
    result["all_folds_present"] = bool(len(fold_frame) == 5)
    return result
...
def main() -> None:
    ...
    if args.smoke_test:
        ...
        if configuration["target_labels"] != TARGET_LABELS:
            raise ValueError("Smoke configuration does not contain exactly the two target labels")
        print(f"SMOKE_CONFIG_OK code_files={sorted(configuration['code_sha256'])}", flush=True)
