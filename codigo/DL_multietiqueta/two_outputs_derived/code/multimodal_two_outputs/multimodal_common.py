...
def select_best_complete_candidate(rows: Iterable[Mapping[str, object]]) -> dict:
    """Select retrospectively using the tuned two-output OOF metrics."""
    complete = []
    for row in rows:
        ...
        try:
            micro = float(row.get("tuned_oof_two_outputs_f1_micro", float("nan")))
            macro = float(row.get("tuned_oof_two_outputs_f1_macro", float("-inf")))
