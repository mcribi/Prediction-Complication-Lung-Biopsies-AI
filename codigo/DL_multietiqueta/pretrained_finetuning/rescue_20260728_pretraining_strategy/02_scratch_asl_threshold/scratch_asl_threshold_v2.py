...
PILOT_CONFIGS = [
    {
        'config_id': 'seresnet50_resize_cube128_multiwindowing_separadas__ct_lung_nodule_vessels__bs8',
        'model_name': 'seresnet50',
        'preprocessing': 'resize_cube128_multiwindowing_separadas',
        'input_name': 'ct_lung_nodule_vessels',
        'batch_size': 8,
        'scratch_oof_f1_micro': 0.5596330275229358,
        'scratch_oof_f1_macro': 0.4981186196967388,
    },
    {
        'config_id': 'resnet10_resize_cube64_multiwindowing_separadas__ct_lung_nodule__bs16',
        'model_name': 'resnet10',
        'preprocessing': 'resize_cube64_multiwindowing_separadas',
        'input_name': 'ct_lung_nodule',
        'batch_size': 16,
        'scratch_oof_f1_micro': 0.5550239234449761,
        'scratch_oof_f1_macro': 0.4600526398279207,
    },
    {
...
def build_pretrained_model(pretraining: str, model_name: str, in_channels: int, out_channels: int, report_dir: Path) -> nn.Module:
    if pretraining == 'scratch_asl':
        model = nested.build_model(model_name, in_channels, out_channels)
        report = {'pretraining': pretraining, 'model': model_name, 'in_channels': int(in_channels), 'out_channels': int(out_channels), 'weights': 'scratch'}
...
def train_one_outer_fold(pretraining, model_name, labels_df, preproc_name, input_cfg, batch_size, train_outer_ids, test_outer_ids, fold_dir, device, fold_seed):
    ...
    model = build_pretrained_model(pretraining, model_name, in_channels, len(TARGET_LABELS), fold_dir).to(device)
    set_encoder_trainable(model, pretraining, False)
    if pretraining == 'scratch_asl':
        freeze_unfreeze_events = [{'epoch': 1, 'encoder_trainable': True, 'note': 'scratch ASL: all parameters trainable from the start'}]
    else:
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        if pretraining != 'scratch_asl' and epoch == FREEZE_EPOCHS + 1:
...
def run_pilot(pretraining: str, run_tag: str = '', config_indices: List[int] | None = None, batch_size_override: int | None = None) -> Path:
    ...
    run_root = OUTPUT_ROOT / f'{pretraining}_threshold_tuning_v2_{timestamp}{suffix}'
    ...
    setup = {
        ...
        'pilot_scope': 'scratch_asymmetric_loss_with_val_inner_threshold_tuning_on_top_scratch_configs',
        ...
        'trainability': 'scratch ASL: all model parameters trainable from epoch 1; no freeze/unfreeze',
        'seed': SEED,
    ...
    for cfg in selected_configs:
        ...
        row = {
            'pretraining': pretraining,
            'model_family': f"{cfg['model_name']}_scratch_asl" if pretraining == 'scratch_asl' else ('resnet34' if pretraining == 'medicalnet' else 'genesis_encoder_classifier'),
        ...
        try:
            for fold_idx, (train_idx, test_idx) in enumerate(split_indices, start=1):
                ...
                fold_summary = train_one_outer_fold(pretraining, cfg['model_name'], labels_df, cfg['preprocessing'], input_cfg, int(cfg['batch_size']), train_outer_ids, test_outer_ids, fold_dir, device, SEED + fold_idx)
