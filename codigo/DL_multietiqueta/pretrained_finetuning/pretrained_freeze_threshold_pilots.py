...
def run_pilot(pretraining: str, run_tag: str = '', config_indices: List[int] | None = None) -> Path:
    ...
    selected_configs = PILOT_CONFIGS
    if config_indices is not None:
        selected_configs = [PILOT_CONFIGS[i] for i in config_indices]

    setup = {
        'timestamp': timestamp,
        'pretraining': pretraining,
        'pilot_scope': 'phase2_freeze_unfreeze_threshold_tuning_on_two_high_macro_configs',
    ...
    for cfg in selected_configs:
        ...
        row = {
            'pretraining': pretraining,
            'model_family': 'resnet34' if pretraining == 'medicalnet' else 'genesis_encoder_classifier',
...
def main():
    ...
    run_pilot(args.pretraining, run_tag=args.run_tag, config_indices=args.config_index)
