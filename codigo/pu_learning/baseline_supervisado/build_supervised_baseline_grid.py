...
def parse_args():
    parser = argparse.ArgumentParser(description="Build initial supervised baseline configs for PU-learning comparison.")
    parser.add_argument("--out_root", type=str, default=str(Path("/mnt/homeGPU/mcribilles/tfm/codigo/pu_learning/baseline_supervisado/baseline_grid")))
...
def build_templates(random_state):
    ...
    add_template(
        templates,
        "extratrees_ovr_none_none_balsub",
        "ExtraTrees",
        "one_vs_rest",
        "none",
        "none",
        {"n_estimators": 500, "max_features": "sqrt", "max_depth": None, "class_weight": "balanced_subsample", "n_jobs": 1, "random_state": random_state},
    )
    add_template(
        templates,
        "rf_ovr_none_none_bal",
        "RandomForest",
        "one_vs_rest",
        "none",
        "none",
        {"n_estimators": 500, "max_features": "sqrt", "max_depth": None, "class_weight": "balanced", "n_jobs": 1, "random_state": random_state},
    )
    add_template(
        templates,
        "gbt_chain_none_varth",
        "GradientBoosting",
        "classifier_chain",
        "none",
        "varth_0.01",
    ...
    add_template(
        templates,
        "mlp_ovr_std_varth_h64",
        "MLP",
        "one_vs_rest",
        "StandardScaler",
        "varth_0.01",
        {"hidden_layer_sizes": [64], "alpha": 0.001, "max_iter": 800, "early_stopping": True, "random_state": random_state},
    )
    return templates
...
def main():
    ...
    configs_path = out_root / "valid_configurations_supervised_baseline.json"
    ...
    summary = {
        ...
        "note": "The runner builds one shared labelset-stratified partition on this common cohort and can reuse it for every feature set.",
    }
