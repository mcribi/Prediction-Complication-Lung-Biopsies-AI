...
def save_beeswarm(values, data, names, label, out_dir, max_display):
    ...
    fig = plt.gcf()
    ax = plt.gca()
    ax.set_xlabel("Valor SHAP (contribución a la salida interna del modelo)")
...
def main():
    ...
    for label in LABELS:
        ...
        raw_names = archive["feature_names"].astype(str).tolist()
        diagnostic = [name for name in raw_names if "diagnostics_" in name.lower() or "diag_" in name.lower()]
        if diagnostic:
            raise ValueError(f"Diagnostic columns detected in {label}: {diagnostic}")
        names = [clean_feature_name(name) for name in raw_names]
        ...
        for _, row in selected.iterrows():
            ...
            idx = int(match[0])
            output_png = output_dir / "individual" / safe_label(label) / f"{outcome}_representative.png"
            output_png.parent.mkdir(parents=True, exist_ok=True)
            title = f"{label} | {outcome} | fold {folds[idx]} | real={y_true[idx]} | predicción={y_pred[idx]} | p={scores[idx]:.3f}"
